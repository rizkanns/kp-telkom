...
class AMController extends Controller
{
    ...
    public function insertPelanggan(Request $request)
    {
		...
		// dd($pelanggan,$proyek);
		return redirect()->route('proyek_single', ['id' => $proyek->id_proyek, 'id_pelanggan'=>$pelanggan->id_pelanggan]);
	}
    ...
    {
		$proyek = Proyek::find($id,$id_pelanggan);
    ...
    {
    	$proyek = DB::table('proyek')->where('id_proyek',$id)->update($request->all(), ['except' => ['id_proyek'] ]);
    	dd($proyek);
    	return redirect()->route('aspek');
    }
